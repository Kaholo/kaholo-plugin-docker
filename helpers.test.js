...
describe("docker plugin helpers test", () => {
  ...
  describe("parseDockerImageString", () => {
    const expectedParsedHostportPairs = [
    ...
    expectedParsedHostportPairs.forEach(([image, expectedHostport]) => {
      expect(helpers.parseDockerImageString(image).hostport).toStrictEqual(expectedHostport);
    });
  });
});
